...
```json
{
  "timestamp_kst": "2026-06-07T07:45:28.970703+09:00",
  "active_process_lines": [
    "   6058     170       00:00 Ss    9.0  0.0 /usr/bin/bash -c source /tmp/hermes-snap-d20dc5f70c79.sh >/dev/null 2>&1 || true builtin cd -- /opt/data/kaggle/kmu-rec-sys-26-steam || exit 126 eval 'set -euo pipefail cd /opt/data/kaggle/kmu-rec-sys-26-steam python3 -m py_compile scripts/opencode_hermes_axis_rejection_loop.py python3 scripts/opencode_hermes_axis_rejection_loop.py --dry-run --max-iters 1 --sleep-between-iters-sec 0 printf '\\''\\nlatest summaries:\\n'\\'' ls -t reports/*opencode_hermes_axis_rejection_loop_summary.json reports/*opencode_hermes_axis_rejection_loop_summary.md | head -4 | xargs -r -n1 sh -c '\\''echo --- $0; wc -c \"$0\"; head -20 \"$0\"'\\''' __hermes_ec=$? export -p > /tmp/hermes-snap-d20dc5f70c79.sh 2>/dev/null || true pwd -P > /tmp/hermes-cwd-d20dc5f70c79.txt 2>/dev/null || true printf '\\n__HERMES_CWD_d20dc5f70c79__%s__HERMES_CWD_d20dc5f70c79__\\n' \"$(pwd -P)\" exit $__hermes_ec"
  ],
  "latest_reports": [
...
- Markdown report: `reports/20260607T074528KST_axis_loop_iter01_opencode.md`
- JSON report: `reports/20260607T074528KST_axis_loop_iter01_opencode.json`
- Optional validation artifacts only under `artifacts/opencode_hermes_axis_loop_20260607T074528KST`
