...
## 캡스톤 2: 3-split 패널 — BLUNT 판정의 독립 실측 검증

gate-floor의 BLUNT 판정은 **단일 split의 bootstrap**(같은 split 재표집 = within-split 분산)에 근거했다.
이를 **진짜 독립 데이터 draw**로 검증: 동일 emb128 4-seed 앙상블을 seed만 다른 3개 uniform split
(42/7/123, 각 4 model-seed = 12 GPU 학습)에 학습시켜 between-split 분산을 실측 (4×V100, 2웨이브).

### 결과
| split | 앙상블 acc | per-model-seed |
|---|---|---|
| seed42 | **0.76505** | [0.76205, 0.76275, 0.76435, 0.76325] |
| seed7 | 0.76095 | [0.75965, 0.75925, 0.76045, 0.75915] |
| seed123 | 0.75995 | [0.76005, 0.76125, 0.75875, 0.76145] |

- **between-split std = 0.00270, range = 0.00510** (vs bootstrap 절대 SE 0.00367)

### 두 가지 정직한 해석 (분리 필수)
1. **게이트 절대수준 무딤 = BLUNT 확증.** 독립 draw 간 앙상블 정확도가 range 0.0051로 흔들림 —
   게이트 임계 0.003과 같은 차수이고 range는 그걸 넘는다. 단일 split 절대 정확도로 +0.003 신뢰 불가.
   (n=3이라 std 점추정은 거칠다 → **차수 판단만 신뢰**, 정밀값 아님.)
2. **그럼에도 이번 세션 closure는 전부 유효.** seed42가 셋 중 가장 높은 draw(0.765)였다 →
   **줄곧 써온 seed42 절대 수치는 낙관 편향**. 하지만 모든 closure는 *같은 split 위 paired 비교*
   (candidate−base)였고 paired에선 data-draw 분산이 상쇄(paired-SE 0.0018). 닫은 효과는 전부 0이거나
   큰 음수라 SE 밖 멀리 → **절대 레벨 편향에도 상대 게이트 판정은 전부 유효.**

### 분산 성분 분해 (최종)
| 성분 | 크기 | 앙상블이 줄이나? |
|---|---|---|
| within-split model-seed (init/학습 확률성) | seed42 range 0.0023 | ✅ 평균으로 상쇄 |
| within-split user 재표집 (paired) | SE 0.0018 | — (paired가 상쇄) |
| **between-split data-draw** | **std 0.0027** | ❌ **앙상블로 못 줄임 → dominant** |

→ **핵심 교훈:** 앙상블/seed는 model-seed 분산만 줄이고 **data-draw 분산은 못 줄인다.** 후자가 dominant이므로
미래 진짜 후보는 반드시 3-split 패널로 게이트해야 한다(이번에 인프라+aggregator 검증 완료, seed42 재현 0.76505 ✓).
public final-2 선택은 실제 LB(emb128 0.77745 > emb64 0.77125, +0.0062 = MDE의 17배)가 이미 결정적으로 확정했다.
