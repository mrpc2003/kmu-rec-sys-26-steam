# KMURecSys26 Steam OpenCode-first no-submit axis loop — 2026-06-07 00:59:22 KST

## 결론

- **새 OpenCode/새 probe는 실행하지 않았습니다.** 기존 validation-only probe인 `UserKNN gated residual fine-grid`가 아직 살아 있어 중복 실행을 피했습니다.
- **UserKNN fine-grid는 아직 RUNNING_UNCLASSIFIED**입니다. PID `18812`가 약 100% CPU로 돌고 있고, 기대 리포트 `reports/20260606T132450KST_userknn_gated_residual_fine.{json,md}`는 아직 생성되지 않았습니다.
- **Jackknife expanded는 이전 tick과 동일하게 FAILED_INCOMPLETE_NO_METRIC_REPORT**입니다. 프로세스는 없고, 기대 리포트가 없으며, 로그는 12줄에서 중단된 상태입니다.
- **Jackknife smoke는 WEAK_SIGNAL 유지**입니다. 최고 평균 Δ `+0.000367`, min Δ `-0.001200`, `2/3` positive, fixes/breaks `252/230`, p `0.3388`로 strict gate 통과가 아닙니다.
...
- OpenCode CLI/auth 확인: `/opt/data/home/.local/bin/opencode`, version `1.15.11`; credential/provider 값은 출력하지 않았습니다.
- 이전 OpenCode loop:
...
- PIDs: `18483`, `18803`, `18804`, `18812`
- 관측 상태: PID `18812`가 약 100% CPU로 실행 중, elapsed 약 `11h34m`
- log: `logs/userknn_gated_residual_fine_20260606T132450KST.log`
- 관측 log lines: `31,215`
- tail 요약: `scripts/userknn_residual_probe.py:114`의 `RuntimeWarning: invalid value encountered in divide`가 계속 출력 중
- 기대 리포트:
...
- artifact dir: `artifacts/userknn_gated_residual_fine_20260606T132450KST`, 파일 `0`개
...
- pid file: `logs/20260606T220406KST_jackknife_uncertainty_boundary_expanded.pid` → `28646`, 현재 not live
...
- artifact dir: `artifacts/opencode_axis_loop_20260606T220406KST/jackknife_uncertainty_boundary_expanded`, 파일 `0`개
...
### 4) Existing aggressive quota runner / quarantine

- PIDs: `7613`, `7630`
- 기존 프로세스이며 이번 tick에서 시작하지 않았습니다.
- 현재 실제 Kaggle submit 프로세스는 `0`개로 관측했습니다.
- `state/aggressive_quota_runner_state.json`에서 public best `0.77825` 및 quarantine families가 유지되어 있음을 확인했습니다.
- 이 tick에서 quarantine/guard logic은 수정하지 않았습니다.

## GPU/CPU snapshot

- GPU0: `0 / 32768 MiB`, util `0%`
- GPU1: `0 / 32768 MiB`, util `1%`
- GPU2: `0 / 32768 MiB`, util `0%`
- GPU3: `4320 / 32768 MiB`, util `2%`; `nvidia-smi` 프로세스 표에는 owner가 보이지 않음
- loadavg: `3.31 3.22 3.11`
- CPU count: `56`

## Artifacts written this tick

- JSON: `reports/20260607T005922KST_improvement_axis_cron_status.json`
- Markdown: `reports/20260607T005922KST_improvement_axis_cron_status.md`
